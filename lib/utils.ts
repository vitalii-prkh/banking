...
// Normalize US state to 2-letter uppercase abbreviation
...
export function normalizeUSState(input: string) {
  if (!input) {
    return input;
  }
  ...
  if (cleaned.length === 2) {
    return cleaned; // assume already a code
  }

  return US_STATE_MAP[cleaned] || cleaned;
}
